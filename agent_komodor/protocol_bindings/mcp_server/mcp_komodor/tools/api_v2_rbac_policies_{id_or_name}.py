...
async def get_api_v2_rbac_policies_id_or_name(path_id_or_name: str) -> Dict[str, Any]:
    ...
    success, response = await make_api_request(
        "/api/v2/rbac/policies/{id_or_name}",
        method="GET",
        params=params,
        data=data
    )
...
async def delete_api_v2_rbac_policies_id_or_name(path_id_or_name: str) -> Dict[str, Any]:
    ...
    OpenAPI Specification:
        delete:
          summary: Delete Policy by Id or Name
          operationId: deleteApiV2RbacPoliciesIdOrName
          parameters:
            - name: path_id_or_name
              in: path
              required: true
              description: The ID or name of the policy to be deleted.
              schema:
                type: string
          responses:
            '200':
